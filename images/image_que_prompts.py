EXTRACTION_SYSTEM = """
You are a helpful academic assistant which scan and extract questions with images given in grade-{grade} {subject} question bank in latex format based on the guidelines below:
You will be given list of lines which contains questions with images ids of images present in the questions in LateX format from the grade-{grade} {subject} question bank. You need to extract all the questions from the text in latex format and return them in the specified format.

Image_id are given in this format like ("\includegraphics[max width=\\textwidth, center]{{"image_id text"}}\\").Extract the images ids.
Image_id are the filenames of the images present in the question text.They are in format ".jpg" .
###CRITICAL INSTRUCTIONS for EXTRACTIONS###:
- Extract all questions which include or ends with "\\includegraphics"  as they contain images with image_ids. For example (" Which type of lenes are shown in given figure (i) and (ii).\\
\includegraphics[max width=\\textwidth, center]{{2025_01_22_a80d3920b13788732731g-164}}"). \includegraphics is available at the end of the question text.
- Extract the questions with images with their images_id from the given text in its original form in LaTeX format.
- Do not extract opitons which include or ends with "\\includegraphics" . Only extract the questions with images and their image_ids.
- Do not extract paragraphs based questions which include or ends with "\\includegraphics". Only extract the questions with images and their image_ids.
- Do not extract paragrapgh based question text which include or ends with "\\includegraphics".
- Extract the image_id for images from the latex "\includegraphics[max width=\\textwidth, center]{{"image_id"}}\\" Only if it its included in the questions.
- Do not extract questions which do not have images .These questions do not have image_ids.
- Do not generate any new questions or image_ids .Only extract the questions and image_ids which are present in the text.
- If no questions with image_ids are available in the provided text, then only give the empty questions output format .\
- Only extract the image_id text only for image_id . Do not give the image_id in the LateX format. 

##Ouput Instructions##:
- The output should be in JSON format questions with images with images ids in LaTeX format
- While returning the output, make sure to: 
    ...
    - If no questions with images are available in the provided text, return the empty questions output format:
    ...
    [
        {
            "question": 
            {
                "question_text": "questions in LaTeX format",
                "image_ids": 
                [
                    {image_1:"image_id text"},
                    ....
                ]
            },
        }
...
EXTRACTION_USER =  """
Extract all questions with images with their image_ids text available in the given chapter text 
Chapter text:
    {chapter_name}: {chapter_text}

Make sure to extract all available questions with images with their image_ids in the chapter text with there descriptions and follow the output format strictly 
"""
...
RULES for mapping ,rephrasing and  LaTeX formating:
    ...
    - Rephrase the questions as questions should not be exact same as original questions . Do not change the concepts of the questions.
    - Never start questions with like ("how.." , "what.." , "why.." etc )
    - Never start questions with like "which images.." as options will be a text always.
    - Only If questions has missing details or data then add the missing data or details to complete the question  Do not change the context or concept of the question and do not generate images ids. 
    - Convert each and every valid question into different types multiple choice questions without options with original latex format .
    - Map each questions with images with a Topic from the given Topic list
    - Always return the output in JSON
    ...
    [
        {
            "question": 
            {
                "question_text": "\\begin{{aligned}} questions in LaTeX format \\end{{aligned}}",
                "image_ids": 
                [
                   { "image_1":"image_id text"},
                    ....
    ...
    [
        {
            "question": 
            {
                "question_text": "\\begin{{aligned}} &\\text{ Which of the following ray diagrams is correct for the ray of light incident on a concave mirror as shown in figure?} \\end{{aligned}}",
                "image_ids": 
                [
                    {"image_1":"2025_01_22_a80d3920b13788732731g-160(1)"},
                    {"image_2":"2025_01_22_a80d3920b13788732731g-160"}
                ]
            },
            ....
...
CLEAN_REPHRASE_USER = """
Convert the given set of questions with images ids in LaTeX format into multiple choice questions without options and map the questions to the relevant topics. Make sure the mapped, and cleaned data is returned in the given output format.

questions : {question_text} 
The list of topic names with their ids: {topic_list}

Make sure you generate good quality questions while following the rules. Return JSON output in the specified format.
"""

MATHEMATICS_SOLUTION_SYSTEM_PROMPT = """
You are an academic teacher of grade {grade} who has all the knowledge of mathematics in grades-{grade} . You are focused on creating explanations for MCQs questions with images's correct answers which provide a solution from the given question.
